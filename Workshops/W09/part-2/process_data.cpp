...
namespace sdds
{
	...
	int ProcessData::operator()(const std::string& target_file, double& avg, double& var) {
		...
		auto avgFactThread = bind(computeAvgFactor, _1, _2, total_items, _3);
		std::vector<std::thread> m_threads;
		for (int i = 0; i < num_threads; i++) {
			m_threads.push_back(std::thread(avgFactThread, data + p_indices[i], p_indices[i + 1] - p_indices[i], std::ref(averages[i])));
		}
		...
		auto varFactThread = std::bind(computeVarFactor, _1, _2, total_items, avg, _3);
		for (int i = 0; i < num_threads; i++) {
			m_threads.push_back(std::thread(varFactThread, data + p_indices[i], p_indices[i + 1] - p_indices[i], std::ref(variances[i])));
		}
		for (auto& thread : m_threads) {
			thread.join();
		}
		...
		if (!oFile) {
			std::cerr << "Error: Unable to open the file " << target_file << " for writing" << std::endl;
